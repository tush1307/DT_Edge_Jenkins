...
node {
  ...
  stage('Anchore Vulnerability Scanning') {
    ...
    echo "DGOSS TESTING TAG USED FOR IMAGE : ${env.BUILD_NUMBER}";
    sh "docker exec anchore anchore analyze --image ${dockerRepo}/${dockerImageName}:${env.BUILD_NUMBER} --imagetype base"
    sh "docker exec anchore anchore audit --image ${dockerRepo}/${dockerImageName}:${env.BUILD_NUMBER} report"
    sh "docker exec anchore anchore query --image ${dockerRepo}/${dockerImageName}:${env.BUILD_NUMBER} list-files-detail all"
    sh "docker exec anchore anchore query --image ${dockerRepo}/${dockerImageName}:${env.BUILD_NUMBER} cve-scan all"
    sh "docker exec anchore anchore toolbox --image ${dockerRepo}/${dockerImageName}:${env.BUILD_NUMBER} show"
  //---------------------------------------
  }
 }
}
